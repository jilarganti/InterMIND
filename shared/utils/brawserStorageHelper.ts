...
/**
 * Manages a flag in local storage.
 * On the first call for a given key, it stores the value and returns undefined.
 * On subsequent calls, it returns the previously stored value.
 * @param key The key to use in local storage.
 * @param valueToStore The value to store if the key is not already present.
 * @returns The stored value if the key was already present, otherwise undefined.
 */
export function manageLocalFlag(key: string, valueToStore: string): string | undefined {
  ...
  // Fallback for environments without localStorage (e.g., SSR initial pass)
  // or if localStorage is disabled. In this case, act as if it's always the first call.
  return undefined
}
