---
title: アカウント管理
description: InterMINDアカウントの登録と管理。Google、Microsoft、またはメールでの登録方法、プロフィール設定の更新、セキュリティ管理について学びます。
---
...
> [!WARNING]
> 基本プロフィール情報へのアクセスを許可したくない場合は、**キャンセル**ボタンをクリックしてください。アカウントにサインインページに戻り、外部メールアドレスを使用してサインアップできます。

4. InterMINDのメインページに移動します。それ以上の設定は必要ありません

### Microsoftアカウントでサインアップ

この方法は、Microsoftアカウントを使用した迅速なアクセスの取得に適しています。Microsoft Edgeブラウザを利用することで最適な体験を実現できます。
...
InterMINDアカウントの作成後、いつでもサインインすることができます。これにより、会議の主催や参加、設定の管理、AI機能へのアクセス、過去の会議とその議事録の確認が可能になります。さらに、会議履歴内でAIアシスタントを活用することもできます。詳細については[会議の作成とスケジュール設定](./creating-meetings)をご覧ください。
...
1. **Google**ボタンをクリックします
   ...
   - システムがGoogleアカウントを識別できない場合、追加認証のためGoogleサインインページにリダイレクトされます
...
1. **Microsoft**ボタンをクリックします
   ...
   - システムがMicrosoftアカウントを識別できない場合、追加認証のためMicrosoftサインインページにリダイレクトされます
2. 認証が完了すると、InterMINDメインページに直接リダイレクトされます
...
InterMINDのユーザープロフィールページでは、ユーザーが個人設定の表示とカスタマイズ、デバイス設定の管理、コミュニケーションオプションの設定、およびアカウント削除を含む重要なアカウント操作を実行する機会を提供します。
...
**直接URL：** [https://intermind.com/profile](https://intermind.com/profile) に移動
...
- Basicユーザーには**プランをアップグレード**ボタンが表示（[料金](./pricing)について詳しく学ぶ）
- Proユーザーには**サブスクリプションをキャンセル**ボタンが表示

**コミュニケーション設定**

- **ニュースレター**：新機能と改善に関する更新を受信
