---
title: 会议界面概览
description: 探索InterMIND直观的会议界面。了解视频布局、工具栏控件、参与者交互和自定义选项。
---

# 会议界面概览

InterMIND界面的设计充分考虑了简洁性、直观性和可访问性。本节详细介绍了实时会议期间可见的关键元素，并提供了用户如何有效与这些元素交互的指导。

## 主界面布局

参加会议时——无论您是主持人、协调员、参与者还是访客——屏幕布局会根据您的角色而有所不同（请参阅[用户角色](./user-roles)部分）。布局包括：

- **顶部状态栏**：此栏显示会议链接、安全状态、录制和翻译指示器，立即提供重要信息
- **中央视频区域**：此区域以网格或侧边栏格式显示参与者，可通过设置进行调整
- **底部工具栏**：此工具栏包含管理会议所必需的主要交互控件
- **侧面板（可选）**：此面板具有聊天、参与者列表和[AI助手](./ai-features#ai-assistant)功能，增强用户交互体验
...
- 更改翻译器语言，便于说不同语言的参与者之间的沟通（参见 [AI Features](./ai-features#online-voice-translation)）
...
- **聊天**：参与者可以使用侧边面板向通话中的所有与会者发送消息，促进沟通与协作
- **固定/不观看**：用户可以通过固定特定参与者或在个人布局中隐藏他们来自定义视图，定制自己的会议体验
...
| **录制**     | 点击开始或停止录制。启动时，所有参与者都会收到可见和可听的通知。此功能仅对主持人和协调员可用（请参阅[用户角色](./user-roles)） |
| **举手**    | 点击举手或放下手，表示您想要发言的意图。举手时，参与者列表中您的姓名旁边会出现手形图标                                                                     |
| **设置**      | 调整音频、视频、布局和背景模糊设置。参与者还可以选择默认的助手提供商并选择首选的翻译语言                                          |
