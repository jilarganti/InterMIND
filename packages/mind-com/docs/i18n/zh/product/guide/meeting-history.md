---
title: 会议历史记录
description: 在一个地方访问您的 InterMIND 会议档案，包含录音、转录、AI生成的摘要和可重复使用的会议链接。
---

# 会议历史记录

会议历史记录页面作为您在登录状态下主持或参加的所有会议的个人档案。它提供对录音、转录、AI助手生成的摘要和可重复使用的会议链接的快速访问 - 所有内容都整合在一个便捷的位置。
...
点击房间 ID 链接可以整理列表，仅显示与相同房间 ID 关联的会议。
...
当点击此操作时，用户将收到通知，提示他们通过点击 **是，删除** 按钮确认删除，或通过点击 **取消** 按钮取消操作。已删除的会议将不再对任何参与者可见。只有主持人或管理员拥有删除会议的权限（参见[用户角色](./user-roles)）。
...
此操作允许在历史页面上拥有会议记录的任何用户加入会议，从而创建具有相同房间 ID 的新记录。
...
这使用户能够从 [AI Assistant](./ai-features#ai-assistant) 获取聊天格式的会议摘要。
...
- 已登录参与者可以访问所参加会议的历史记录；但是，他们无法删除会议记录
