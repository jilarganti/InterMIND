...
**减少攻击面**，无运行时服务器或数据库漏洞，静态文件消除了 SQL 注入和服务器端攻击向量，基于 CDN 的分发提供 DDoS 保护和全球冗余。

**成本效益**通过 CDN 托管实现，比传统服务器托管显著便宜，无需插件和服务器管理降低运营成本，通过 CDN 分发实现自动扩展，以及无服务器函数使用减少后端维护开销。
