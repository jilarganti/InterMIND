...
**TypeScript 통합**에는 완전한 TypeScript 지원과 검증을 위한 Zod 스키마를 포함한 새로운 @vercel/sdk 패키지, 상세한 타입 정보가 포함된 구조화된 오류 응답, 그리고 서버리스 환경에서 매개변수 처리를 위한 확장된 NextResponse 객체가 포함됩니다.
