...
우리는 **약속보다는 투명성**을 믿습니다. 우리가 직접 제어할 수 있는 구성 요소 — 우리의 소프트웨어, 인터페이스 로직, 그리고 데이터 라우팅 방식 — 에 대해서만 프라이버시와 보안을 보장할 수 있습니다. 그 외의 모든 것에 대해서는 **선택의 권한**을 제공합니다.
...
- 귀하의 세션에서 어떤 지역이 활성화되어 있는지, 어떤 법률이 적용되는지(예: 유럽의 GDPR, 미국의 CCPA, UAE의 PDPL) 명시적으로 확인할 수 있습니다.
...
귀하가 선택하지 않는 한 회의, 전사본 또는 대화를 저장하지 않습니다. 모든 저장소는 지역별로 제한됩니다.

## 법적 및 규제 준수

InterMind는 다음과 완전히 호환됩니다:
...
- 제3자 모델에 전달된 콘텐츠가 해당 모델의 훈련 범위를 벗어날 것 (개인 계약 하에 있지 않는 한)
...
기밀성 요구사항에 맞게 세션을 사용자 정의할 수 있습니다:
...
- 여러분이 지역을 선택합니다.
- 여러분이 해석 수준을 선택합니다.
- 여러분이 감수할 위험을 결정합니다 — 그리고 우리는 모든 수준에서 규정 준수를 유지할 수 있도록 도와드립니다.
