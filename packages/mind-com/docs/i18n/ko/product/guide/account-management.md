---
title: 계정 관리
description: InterMIND 계정을 가입하고 관리하세요. Google, Microsoft 또는 이메일로 등록하는 방법, 프로필 설정 업데이트 및 보안 관리 방법을 알아보세요.
---
...
계정 생성은 세 가지 방법으로 수행할 수 있습니다: Google 또는 Microsoft 계정을 이용하거나 외부 이메일 주소로 등록하는 방법입니다. 전체 개요는 [시작하기](./getting-started) 가이드를 참조하세요.
...
3. 계정 로그인 페이지에서 다음 옵션들을 확인할 수 있습니다:
   ...
   - 이미 계정이 있는 경우 로그인을 위한 이메일 및 비밀번호 필드가 뒤따르는 '또는 다음으로 로그인' 텍스트
   - 외부 이메일 주소로 등록할 수 있는 '회원가입' 링크가 뒤따르는 '계정이 없으신가요?' 텍스트
...
> [!WARNING]
> 기본 프로필 정보에 대한 접근 권한을 부여하지 않으려면 **취소** 버튼을 클릭하세요. 계정 로그인 페이지로 다시 이동되며, 외부 이메일 주소를 사용하여 회원가입할 수 있습니다.

4. InterMIND 메인 페이지로 이동되며, 추가 설정은 필요하지 않습니다

### Microsoft 계정으로 회원가입

이 방법은 Microsoft 계정을 사용하여 빠른 접근을 얻기 위해 권장됩니다. Microsoft Edge 브라우저를 사용하면 최적의 경험을 얻을 수 있습니다.
...
InterMIND 계정을 생성하면 언제든지 로그인할 수 있습니다. 이를 통해 회의를 주최하거나 참여하고, 설정을 관리하며, AI 기능에 액세스하고, 이전 회의와 회의록을 검토할 수 있습니다. 또한 회의 기록 내에서 AI 어시스턴트를 활용할 수 있습니다. [회의 생성 및 예약](./creating-meetings)에 대해 자세히 알아보세요.
...
2. 인증이 완료되면 InterMIND 메인 페이지로 바로 이동됩니다
...
4. **로그인**을 클릭하여 진행합니다
5. 인증이 완료되면 InterMIND 메인 페이지로 바로 이동됩니다
...
- Basic 사용자는 **플랜 업그레이드** 버튼을 볼 수 있습니다([요금제](./pricing)에 대해 자세히 알아보기)
...
- **회의**: 0 / 25 (Pro 사용자의 경우 무제한)
- **온라인 참가자**: 최대 100명 (Pro 사용자의 경우 150명)
- **기간 시작** 및 **기간 종료** 열

**송장**
발행된 송장 목록을 다음과 함께 표시합니다:
...
> [!INFO]
> 거래가 발생하지 않은 경우 송장 및 결제 섹션이 비어 있을 수 있습니다.

#### 설정

왼쪽 사이드바의 설정 탭을 통해 접근할 수 있습니다. 이 탭을 통해 사용자는 장치 및 회의 기본 설정을 구성할 수 있습니다.
...
> [!TIP]
> 변경 사항은 자동으로 저장되며 향후 모든 회의에 적용됩니다.
...
> [!DANGER]
> 삭제된 계정은 복구할 수 없습니다. 귀하의 이름은 회의 기록에서 삭제된 사용자로 표시됩니다.