---
title: 시작하기
description: InterMIND AI 기반 화상회의 플랫폼 사용법을 알아보세요. 회원가입, 회의 생성, AI 기능, 문제 해결에 대한 완전한 가이드입니다.
---

# 시작하기

InterMIND는 다양한 언어, 기기, 팀 간의 원활한 소통을 촉진하도록 설계된 고급 AI 기반 화상회의 플랫폼입니다. 이 가이드는 첫 회의 참여부터 AI 도구 구성 및 기술적 문제 해결까지 InterMIND의 모든 기능을 효과적으로 활용하는 방법에 대한 포괄적인 이해를 제공하는 것을 목표로 합니다.

자세한 안내가 필요한 초보 사용자든 복잡한 회의를 관리하는 숙련된 참가자든, 이 가이드는 필요한 모든 정보를 접근하기 쉬운 한 곳에 통합했습니다.

이 가이드에서 다음을 확인할 수 있습니다:

- [회원가입, 로그인 및 프로필 사용자 정의](./account-management) 방법에 대한 안내
- [회의 생성 및 예약](./creating-meetings)에 대한 가이드라인
- [사용자 역할](./user-roles), 권한 및 [실시간 번역 기능](./ai-features#online-voice-translation)에 관한 정보
- AI 어시스턴트 및 음성 번역기와 같은 [AI 기반 기능](./ai-features) 개요
- [문제 해결 팁](./troubleshooting) 및 [자주 묻는 질문](./faq) 답변

시작해 보겠습니다 — 지능형 회의의 미래가 지금 시작됩니다.

## 빠른 시작 체크리스트

InterMIND를 처음 사용하신다면, 다음 간단한 체크리스트를 따라 시작해보세요:

1. https://intermind.com 방문
2. 로그인을 클릭하고 Google / Microsoft / 이메일 중 선택 ([계정 관리](./account-management)에서 자세히 알아보기)
3. 메시지가 표시되면 마이크와 카메라 액세스 허용
4. 새 회의를 클릭하고 즉석 회의 시작 선택 ([회의 생성 및 예약](./creating-meetings) 참조)
5. 참가자들과 회의 링크 공유
6. 설정을 열어 선호하는 언어와 장치 선택
7. 통화 중 메모 작성과 요약 생성을 위해 [AI 어시스턴트](./ai-features#ai-assistant) 사용
8. 녹화 및 대화록을 위해 기록 탭을 통해 이전 회의 액세스

> [!TIP]
> 최상의 성능을 위해 Chrome 또는 Edge 브라우저 사용을 권장합니다. 문제가 발생하면 [문제 해결](./troubleshooting) 가이드를 확인하거나 [도움말 및 지원](./help-support)을 방문하세요.
...
사용자는 별도의 소프트웨어 설치 없이 브라우저를 통해 직접 플랫폼에 접속할 수 있습니다.
...
- Chrome (Google의 공식 웹 브라우저)
- Edge (Microsoft의 공식 웹 브라우저)
- Safari (Apple의 공식 웹 브라우저)
