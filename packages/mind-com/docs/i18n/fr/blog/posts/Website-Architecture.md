...
Les **événements dataLayer personnalisés** pour le suivi d'expérimentations utilisent la structure `{'experimentId': 'id', 'variationId': 'id'}`, garantissant un suivi précis des variantes de test sans impact sur les performances.
...
**Monorepo avec pnpm** offre une vitesse d\'installation supérieure : npm (~45s), yarn (~35s), pnpm (~22s), avec 85 MB d\'espace disque partagé total au lieu de 130 MB par projet pour npm.
